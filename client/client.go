...
func (c *Client) refreshAccessToken() {
	result, err := Get(fmt.Sprintf("%s/cgi-bin/token", c.baseURL), map[string]string{
		"appid":      c.AppID(),
		"secret":     c.AppSecret(),
...
func (c *Client) refreshJsAPITicket() {
	result, err := Get(fmt.Sprintf("%s/cgi-bin/ticket/getticket", c.baseURL), map[string]string{
